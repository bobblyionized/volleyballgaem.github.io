...
function blockSolve(b, pos, nd, tz, tx, right) {   // a block touch on ball b by someone at pos facing the net along nd: W tilt (tz>0) = kill block, S tilt = soft one-touch back to our side
  const s = b.vel.length(); let vel, g = 1;
  if (tz >= 0) {
    ...
    else { const tg = pos.clone().addScaledVector(withLateral(nd, tx, 30), 5.5); tg.y = 0; vel = launchTo(b.pos, tg, b.pos.y + 3.6); }
  } else {
    const f = clamp((s - 6) / 22, 0, 1); const dist = 1.5 + 6.5 * f;
    const tg = pos.clone().addScaledVector(nd, -dist).addScaledVector(right, tx * 2); tg.y = 0;
...
function blockContact() {
  ...
  const tz = P.tilt.y * (hasTrait('b1p2') ? -1 : 1), tx = P.tilt.x;   // Fake Block: forward / back tilt swapped
  const r = blockSolve(B, P.pos, netDir(), tz, tx, cr());
  hitBall('block', r.vel, r.g);
}
...
function spikeGeomAt(pos, fwd, tz) {           // the geometry of a spike from `pos` aimed along `fwd` (shared by the player and the bots)
  let toNet = netAhead(pos, fwd); if (!isFinite(toNet)) toNet = 7;
  const CL = S.scene === 'match' ? courtDims().l : COURT_L; const dMid = clamp(toNet + CL / 4, 3, 24);
  const clearPitch = toNet < 1.2 ? -Math.PI / 2 : Math.min(35 * D, Math.atan2((NET_H + 0.35) - pos.y, toNet));   // capped; right at the net TOO LOW handles it instead
  const neutralPitch = Math.max(Math.atan2(-pos.y, dMid), clearPitch);
...
function spikeGeom() { return spikeGeomAt(B.pos, P.jumpFwd.clone().normalize(), P.tilt.y); }   // aim = facing at the moment you jumped; tilt never steers it
function spikeSolve(pos, fwd, tz, c, mult = 1) {   // the spike itself: velocity + gravity for a swing at charge c (0..1) with tilt tz - lob means it would have gone into the net (TOO LOW)
  const { neutralPitch, clearPitch, toNet, far } = spikeGeomAt(pos, fwd, tz);
  ...
  if (far > 0) {                                                        // far from the net: aim higher with more gravity so every tilt can still land in
    ...
    let pf = ballisticPitch(sp, gg, R, -pos.y); if (pf === null) pf = 40 * D;
    for (let i = 0; i < 6; i++) {                                       // make sure it clears the net on the way
      const t = toNet / (sp * Math.cos(pf)); const y = pos.y + sp * Math.sin(pf) * t - 0.5 * gg * t * t;
  ...
  if (isFinite(toNet) && toNet < 40) {
    const tN = toNet / Math.max(0.5, sp * Math.cos(pitch)); const yN = pos.y + sp * Math.sin(pitch) * tN - 0.5 * BALL_G * g * tN * tN;
    if (yN < NET_H + BALL_R) {
      const tg = pos.clone().addScaledVector(fwd, toNet + 4.5); tg.y = 0;
      return { vel: launchTo(pos, tg, Math.max(pos.y + 4.5, NET_H + 3.5)), g: 1, lob: true };
    }
  }
  return { vel: new V3(fwd.x * Math.cos(pitch) * sp, Math.sin(pitch) * sp, fwd.z * Math.cos(pitch) * sp), g, lob: false };
}
function doSpike(c) {
  ...
  const r = spikeSolve(B.pos, fwd, tz, c, P.doubleSpike ? 1.2 : 1);   // Double Spike hits 20% harder
  hitBall('spike', r.vel, r.g, c); if (r.lob) showBallMsg('TOO LOW', B.pos); return true;
}
function doTip() {
...
function updateBalls(dt) {
  for (const b of balls.values()) {
    ...
    const mine = b.mesh.getObjectByName('mine'); if (mine) mine.visible = !b.held && b.hitter === SID && b.hitType !== 'toss';   // the ball turns red while your touch was the last one
  }
...
function makeBot(id, name, model, team, idx, n) {
  ...
  const bot = { id, name, model, team, idx, n, rig, tag, pos: spawnPos(team, idx, n), vel: new V3(), onGround: true, ry: team === 'A' ? 0 : Math.PI, target: null, face: null, role: 'player', plan: null, jumpAt: 0, blockUntil: 0, blockTilt: -1, spike: null, lastHitT: -9, serveAt: 0, coin: 0, base: null };
...
function botStep(bot, dt) {
  ...
  if (bot.target && bot.onGround) {
    ...
    const top = bot.role === 'setter' || (bot.plan && bot.plan.startsWith('set')) ? BOT_SPEED * 1.3 : BOT_SPEED;   // setters are quick on their feet
    if (d > 0.12) { const sp = Math.min(top, d / dt); bot.vel.x = dx / d * sp; bot.vel.z = dz / d * sp; } else { bot.vel.x = bot.vel.z = 0; }
...
function botJump(bot, straight = false) { if (!bot.onGround) return; if (straight) { bot.vel.x = bot.vel.z = 0; } /* straight: no carry-over run, it goes up on the spot (sets and blocks) */ bot.vel.y = JUMP_V; bot.onGround = false; bot.rig.impact(0.14); bot.rig.base = 'air'; bot.rig.setPose('jumpUp', T + 0.18); }
...
function botPickSpike(bot, from) {               // decided the moment the hitter leaves the ground: one of six hits, aimed from where the ball will be, and it stays that way in the air
  const kinds = ['leftCorner', 'rightCorner', 'middleLong', 'cut', 'straight', 'tip']; const kind = kinds[Math.floor(Math.random() * kinds.length)];
  const tg = spikeTarget(kind, bot.team, from);
  const fwd = new V3(tg.x - from.x, 0, tg.z - from.z); if (fwd.lengthSq() < 1e-4) fwd.set(0, 0, -sideZ(bot.team)); fwd.normalize();
  const tz = kind === 'cut' ? 0.55 : kind === 'straight' || kind === 'tip' ? 0 : -0.5;                // sharp cut = W tilt (steep), corners and the long ball = S tilt (deep), straight = neutral
  bot.spike = { kind, tg, fwd, tz, c: 0.5 + Math.random() * 0.5 };                                     // 50-100% power
  bot.face = tg.clone();
}
function botSpike(bot) {
  const b = matchBall(); if (!bot.spike) botPickSpike(bot, b.pos);
  const sp = bot.spike; bot.spike = null; bot.face = sp.tg.clone();
  if (sp.kind === 'tip') {                                                                               // a tip: the same soft touch a player's tip is
    const pitch = 32 * D, s = 7; botHit(bot, 'tip', new V3(sp.fwd.x * Math.cos(pitch) * s, Math.sin(pitch) * s, sp.fwd.z * Math.cos(pitch) * s), 1); return;
  }
  const r = spikeSolve(b.pos, sp.fwd, sp.tz, sp.c);                                                     // exactly the player's spike: same speed, pitch, gravity and TOO LOW rule
  botHit(bot, 'spike', r.vel, r.g); if (r.lob) showBallMsg('TOO LOW', b.pos);
}
/* ---- serving ---- */
  ...
  if (incoming && oursTouches < 3) {
    ...
    if (oursTouches === 0) {                                                                            // RECEIVE: nearest to the landing (not the setter) runs there and sets it up
      ...
      if (botReach(recv, false) && botMayTouch(recv) && (b.vel.y < 1 || b.pos.y < 1.6)) {              // touch 1: up to the setter spot
        ...
        botHit(recv, b.pos.y < 1.0 ? 'bump' : 'set', launchTo(b.pos, tg, Math.max(7.5, b.pos.y + 3.75)), 1);   // a high pass so the setter can jump-set it
      ...
      const at = hi || predictBall(b, { height: 2.1 }).atH; const under = at ? new V3(at.x, 0, at.z) : new V3(pred.land.x, 0, pred.land.z);
      if (st.onGround) { st.target = under.clone().addScaledVector(new V3(0, 0, sz), 0.25); st.face = new V3(b.pos.x, 0, b.pos.z); }
      const wantJump = hi && dist2(st, under) < 0.5 && hi.t < 0.5 && b.pos.y > 3.0;                     // right under it and the ball is half a second from the hands: go straight up
      if (wantJump && st.onGround) botJump(st, true);                                                    // too far to get under it in time: it stays down and sets the ball on the ground instead
      if (botMayTouch(st) && botReach(st, !st.onGround) && (!hi || !st.onGround || (b.vel.y < 0 && b.pos.y < 2.6))) {   // in the air: set at the top; on the ground only if it never got high enough
      ...
      if (hitter) {
        hitter.plan = 'attack'; hitter.target = aim.clone().addScaledVector(new V3(0, 0, sz), 0.45);
        if (hitter.onGround) { hitter.face = new V3(b.pos.x, 0, b.pos.z); hitter.spike = null; }      // on the ground it watches the ball; once it is up the hit is locked in
        const tLeft = spikeAt ? spikeAt.t : pred.land.t;
        if (hitter.onGround && spikeAt && tLeft < 0.62 && dist2(hitter, aim) < 1.2) { botPickSpike(hitter, new V3(spikeAt.x, spikeAt.y, spikeAt.z)); botJump(hitter); }
    ...
    if (theirs >= 2 && b.pos.z * sz < 0) {                                                              // they have set: front the attacker
      ...
      const netX = predictBall(b, { height: 2.9 }).atH; const aimP = netX ? new V3(netX.x, 0, netX.z) : new V3(pred.land.x, 0, pred.land.z);
      const atk = []; if (!human && b.hitter !== SID) atk.push({ pos: P.pos, up: !P.onGround }); for (const x of BOTS) if (x.team !== team && x.id !== b.hitter) atk.push({ pos: x.pos, up: !x.onGround });
      const attacker = atk.length ? atk.reduce((a, x) => x.pos.distanceTo(aimP) < a.pos.distanceTo(aimP) ? x : a, atk[0]) : null;   // whoever is closest to where the set comes down (never the one who just set it)
      const attackerPos = attacker ? attacker.pos : null; const fx = netX ? netX.x : (attackerPos ? attackerPos.x : b.pos.x);
      const cands = setter ? [setter] : bots.filter(x => !(human && x.coin < 0.5 && bots.length === 1));   // 3s+: only the setter ever goes up to block; 2s: a lone teammate flips a coin, block or dig
      ...
      if (blocker) {
        ...
        const attackerUp = !!attacker && attacker.up;                                                    // go up with the hitter (not with their setter's jump set)
        if (attackerUp && blocker.onGround && Math.abs(blocker.pos.z) < 1.6) { blocker.blockTilt = (size <= 2 && Math.random() < 0.5) ? 1 : -1; botJump(blocker, true); blocker.blockUntil = T + 1.0; blocker.rig.base = 'block'; blocker.rig.setPose('block'); }   // 2s: kill block (W) or one-touch (S) 50/50; 3s+: bots only one-touch
      }
    }
    for (const bot of bots) {
...
function botBlockContacts() {                    // a ball coming over the tape within reach of a jumping blocker goes straight back
  ...
  for (const bot of BOTS) {
    ...
    const nd = new V3(0, 0, -sz); const tx = (Math.random() - 0.5) * 0.6;                              // straight at their court, a touch of lateral
    const r = blockSolve(b, bot.pos, nd, bot.blockTilt || -1, tx, new V3(-1, 0, 0).multiplyScalar(sz));
    bot.face = bot.pos.clone().addScaledVector(nd, 3);
    botHit(bot, 'block', r.vel, r.g); bot.blockUntil = 0;
  }
}
