...
function planTeam(team, bots) {
  ...
  if (incoming && oursTouches < 3) {
    ...
    if (oursTouches === 0) {                                                                            // RECEIVE: nearest to the landing (not the setter) runs there and sets it up
      ...
      const wantJump = hi && dist2(st, under) < 0.7 && hi.t < 0.5 && b.pos.y > 3.0;                     // in position and the ball is half a second from the hands: go up
    ...
    if (theirs >= 2 && b.pos.z * sz < 0) {                                                              // they have set: front the attacker
      ...
      if (blocker) {
        ...
        if (attackerUp && blocker.onGround && Math.abs(blocker.pos.z) < 1.6) { blocker.blockTilt = (size <= 2 && Math.random() < 0.5) ? 1 : -1; botJump(blocker); blocker.blockUntil = T + 1.0; blocker.rig.base = 'block'; blocker.rig.setPose('block'); }   // 2s: kill block (W) or one-touch (S) 50/50; 3s+: bots only one-touch
      }
    }
    for (const bot of bots) {
